...
def test_create_salon_duplicate_slug_returns_409_and_is_atomic(client, db_session):
    token = _platform_token()
    with patch("app.routers.salons.geocode_address", return_value=(6.9271, 79.8612)):
        first = client.post(
            "/admin/salons",
        ...
        second = client.post(
            ...
            json={
                "slug": "glamour-lk",
                "name": "Glamour Salon Duplicate",
                "category": "unisex",
                "address": "456 Galle Rd",
                "city": "Colombo",
                "admin_email": "other-owner@glamour.lk",
                "admin_password": "secret123",
            },
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    assert db_session.query(Salon).count() == 1
